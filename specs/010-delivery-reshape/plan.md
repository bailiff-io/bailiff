...
- `scripts/clerk.py` drives the **full lifecycle** — `discover`, `trust`, `init`,
...
- The clerk-specific safety gates keep their constitutional homes: the
  ...
  **discovery** (Constitution VI) and re-checked before `init` writes; the **trust
  ...
  `clerk.py trust`, with copier itself the authoritative gate at render time.
...
- The copier-only guarantee is preserved **for free**: because `clerk.py reproduce`
  only issues plain `copier recopy` commands, a human with copier but no clerk (and
...
copier, only re-packages clerk). Every verb is `scripts/clerk.py <verb>`:
...
- **init** `--run-spec <file> [--check]` — drives `run_copy(data=…, defaults=True,
  ...
  is copier's own `--pretend` dry run. Writes NO clerk artifact.
...
- **copier-only fallback** (documented in SKILL, not a clerk verb): `copier recopy
  --vcs-ref=:current: --defaults --overwrite` per answers file, run by hand with no
  clerk/just — the exact commands `clerk.py reproduce` issues.
...
`clerk` console script now invoke `scripts/clerk.py <verb>` across the full loop
...
**copier-only-by-hand** fallback (plain `copier recopy` with no clerk/just) and
asserts it matches `clerk.py reproduce` byte-for-byte (US1). `mypy --strict` +
...
|---|---|---|
| I — Skills + templates + minimal glue | PASS | Deliverable is `SKILL.md` + one bundled `scripts/clerk.py` driving copier's public API once per template layer (never re-implementing it). Removes `[project.scripts] clerk`. Uniform 1..N path — no speculative single-template branch. Strictly *reduces* glue vs 001. |
| II — Two-phase; skill conducts, helpers execute | PASS | Skill authors the run-spec; the deterministic phase is `scripts/clerk.py` (init/reproduce/discover/trust), runnable/testable with no LLM. Agent never in reproduce. |
| III — Faithful, agent-free reproduce | PASS | `clerk.py reproduce` drives `run_recopy(vcs_ref=CURRENT, …)` per answers file; bare recopy never used; multi-template order recomputed (003), never frozen. The exact commands run by hand without clerk reproduce identically. Determinism test asserts byte-identity. |
...
| V — Determinism via pinning; trust by source | PASS | `today` injected via `data=`; trust in `settings.yml` (expanded-https), written only by `clerk.py trust` on consent; copier authoritatively refuses untrusted sources (clerk pre-checks to name the prefix); CI fails loudly. |
...
| VII — Hardening per-step (scaled) | PASS | DoD = byte-identical reproduce test (both via `clerk.py reproduce` and the by-hand copier-only fallback) + error surfacing (`clerk.py` translates copier errors + surfaces `DiscoveryError`/`UntrustedSourceError`) + adapted loop/unit tests. No adapter → no drift test. |
...
│                        #   ONE uniform 1..N path (N=1 = single template). Reuses src/clerk/*.
└── try-clerk.sh         # UPDATED: inspectable walkthrough using scripts/clerk.py end-to-end +
                         #   a copier-only-by-hand reproduce finale (drop the justfile step and
                         #   the installed-clerk framing).

skills/clerk/
└── SKILL.md             # REWRITTEN: phase-1 procedure invoking `./scripts/clerk.py
                         #   discover|trust|init|reproduce`; documents the copier-only reproduce
                         #   fallback; removes all `clerk`-console / `just reproduce` refs.

src/clerk/               # Retained library logic (imported by scripts/clerk.py):
...
│                        #   translation + reproducibility/trust pre-checks). Used by clerk.py init &
...
└── cli.py               #   REMOVED — its verbs move into scripts/clerk.py (which imports the same
                         #   discovery/trust/runner libs). Delete once scripts/clerk.py subsumes it;
...
tests/
├── loop/                # ADAPTED (not weakened): invoke scripts/clerk.py for the whole loop; same
│   │                    #   assertions. Reproduce also verified by hand with copier-only (US1).
│   ├── test_init.py                  # `clerk.py init` (N=1); asserts recorded answers, NO justfile/clerk file written
│   ├── test_reproduce.py             # `clerk.py reproduce` byte-identical + the by-hand `copier recopy :current:` fallback (no clerk/just)
│   ├── test_check.py                 # `clerk.py init --check` (copier --pretend) writes nothing; surfaces invalid answers
│   ├── test_trust_refusal.py         # action-taking untrusted source refused; consent via clerk.py trust → success
...
the single bundled `scripts/clerk.py`, which drives the full lifecycle through one
uniform 1..N path. `src/clerk/{discovery,trust,errors,runner}.py` are retained as
...
is removed (its verbs move into `scripts/clerk.py`). No new module, no adapter, no
...
at N=1**. `clerk.py reproduce` enumerates the committed answers file(s) and drives
`copier recopy --vcs-ref=:current:` per layer; at N=1 that is one file → one recopy.
This resolves the earlier "what does `clerk.py` expose" question: it exposes the
...
reproduce (US1) remains the guarantee/fallback because `clerk.py` only ever issues
plain copier commands.

## Complexity Tracking

No constitutional violations. This plan reduces surface versus the merged 001 code
(removes: the `clerk` console script, the generated justfile + its writer, `cli.py`)
