...
**Storage**: Files only. The built marketplace manifests are committed
(`marketplace.claude.output` / `marketplace.codex.output` paths in `apm.yml`,
e.g. `dist/marketplace.json` + a codex equivalent). The vendored `src/clerk` copy
lives under the package skill dir. Nothing is written into a *consumer's* project by
packaging itself.
...
live `apm` CLI (`apm pack` emits `marketplace.claude.output`/`marketplace.codex.output`;
`apm publish` uploads to a registry; `apm marketplace init/package/validate` manage
the block; `type: hybrid`/`target: all`/`includes: auto` from the `secrets-scan`
reference package). No separate research.md needed.

### Source Code (repository root)

```text
apm.yml                  # EDIT: add a `marketplace:` block (apm marketplace init + package add) with
                         #   claude + codex outputs + a versioning strategy. Declare the clerk skill package.
...
.apm/skills/clerk/       # NEW (packaged layout, generated): SKILL.md + scripts/clerk.py + vendored clerk/*.py.
                         #   Produced by the vendoring sync step; drift-checked against src/clerk.
.claude-plugin/plugin.json  # NEW: the Claude plugin manifest (name/version/description/skills path),
                         #   matching the secrets-scan reference shape.

justfile                 # EXTEND: `just vendor` (copy src/clerk → package skill dir) + `just check-vendor`
                         #   (fail on drift) + `just pack` (apm pack -m claude,codex) + `just release`
                         #   (apm pack --check-versions --check-clean → apm publish, documented/gated).
...
└── loop/
    └── test_packaging.py   # NEW: apm.yml has a valid marketplace block (claude+codex outputs);
                            #   `apm pack -m claude,codex --dry-run` succeeds; vendored-drift check;
                            #   `clerk doctor` subprocess exit codes. (apm-dependent parts marked/guarded.)
```
