# Feature Specification: clerk Single-Module Vertical Slice

**Feature Branch**: `001-clerk-vertical-slice`
...
clerk is an agentic conductor for the copier scaffolding engine. This first slice
proves the entire clerk value loop end-to-end at the smallest honest scale: one
...
**Why this priority**: This is the whole reason clerk exists — turning a template
plus human intent into a concrete, reproducible project. Everything else in the
roadmap builds on this loop. If only this story ships, clerk already delivers a
...
**Why this priority**: Faithful, assistant-free reproduction is clerk's headline
guarantee and the reason the whole two-phase split exists. A generator that cannot
reproduce its output is not clerk.
...
control). Templates that take actions can, in general, run arbitrary code, so clerk
...
is clerk's core promise, the tool detects this during inspection and refuses to
...
- **FR-027**: The project's own documentation MUST be corrected wherever it
  ...
  rendering happens "without trust, so all action-taking stays in clerk's
  ...
  clerk never itself renders or executes template actions (Principle I); (c) the
  dependency-manifest comment asserting clerk "never runs template actions (no
...
- **Example template ("clerk-mod-base")**: the concrete artifact under test — renders
  ...
  split apart in the project-setup port (roadmap spec 009). As a clerk-authored
...
- **SC-006**: Every input problem the person or assistant sees is a typed, structured
  clerk error; no raw engine stack trace reaches them for the covered cases (missing
...
- **The person consents to trust out-of-band.** clerk explains the implication and records
  the decision, but the decision itself is the human's; clerk never decides to trust.
