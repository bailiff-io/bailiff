...
- `clerk-mod-dep-updates` = ONE module with the `dep_update_tool [renovate, dependabot]`
...
|---|---|---|
| clerk-mod-devcontainer | Build (P1); pure managed render deriving from frozen `mise_tools` | Zero drift between host pins and container |
| clerk-mod-editorconfig | Build (P2); micro-module, deliberately NOT part of base | Keeps base thin; sections derive from frozen linter facts |
| clerk-mod-cocogitto | Build (P1); dogfooded (clerk runs on cog); release-please is a later sibling | Disjoint renders = sibling split (FR-001) |
| clerk-mod-dep-updates | Build (P1); ONE module with `dep_update_tool` axis; default follows repo host | Isomorphic family; host-native default (dependabot on GitHub, renovate otherwise) |
| clerk-mod-moon | Build (P2); turbo/nx are later siblings | Monorepo tools "too distinct" for an axis; closes the dangling `monorepo_tool` CI read |
| clerk-mod-mkdocs | Build (P2); vitepress is a later sibling | Per-engine split, not an axis |
| clerk-mod-gitlab-repo | Build (P2); exact semantic port of github-repo to `glab` | Host parity; consent gate is a safety property, ported faithfully |
| clerk-mod-api | Build (P3); seed-once OpenAPI skeleton + managed spectral config | API-first without codegen or server scaffold |
...
|---|---|---|
| FR-009: remove `dependabot.yml` from `clerk-mod-base` | Dependency-update policy is owned by exactly one module (`clerk-mod-dep-updates`); base ships one clean v1.0.0 | MUST land before the 011 Phase 7 publish batch — **already merged (commit a68295e)**; only spec-artifact annotations remain |
| FR-010a: CI modules accept `monorepo_tool=moon` | Moon is the first real monorepo-tool supplier; the CI modules' `monorepo-affected` model must accept it before moon exists | MUST land before `clerk-mod-moon` is authored (plan Phase 2 before Phase 4) |

## Naming decision (ratified 2026-07-14)

FR-014: `clerk-mod-justfile` KEEPS its name — the maintainer rejected a preemptive rename
to `clerk-mod-runner`. Reconsidered only if/when make/task ship as a monolith with a
...
| FR-011 mkdocs pin strategy | **`mise_tools` contribution** (`mkdocs`, `mkdocs-material` tokens), regardless of `clerk-mod-python` co-selection | Every non-runtime tool pins through the `mise_tools` frozen union; a Python-dev-deps special case would introduce conditional pin behavior and a second mental model. |
...
`clerk-mod-api` (T010) has no dependency on any Slice B module — it moves into the
...
- `clerk-mod-release-please`, `clerk-mod-turbo`, `clerk-mod-nx`, `clerk-mod-vitepress`
...
- Constitution / C-11 / roadmap changes (spec 013 owns the engine relaxation; C-11 holds
  unchanged for 012)
- Capability declarations (`_clerk_provides` / `_clerk_exclusive`) are spec 013 engine
