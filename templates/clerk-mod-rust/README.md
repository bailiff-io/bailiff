...
[`clerk-mod-base`](https://github.com/copier-clerk/clerk-mod-base) (spec 011).
...
- `run_after: [clerk-mod-base]` (hidden `when:false` answer).
...
- **`hook_blocks`** → clippy `pre-commit-hooks-rust` block at `clippy_stage`; `clerk-mod-precommit` is the single hook-config writer.
...
```sh
# Multi-layer (preferred — via clerk):
uv run scripts/clerk.py init --run-spec <run-spec with [clerk-mod-base, clerk-mod-rust]>

# Standalone (renders with defaults; cargo must be on PATH):
copier copy --trust https://github.com/copier-clerk/clerk-mod-rust.git <destination>
```
